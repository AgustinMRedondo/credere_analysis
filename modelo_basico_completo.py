...
def mb_completo():
    ...
    with st.sidebar:
        ...
        m2 = st.number_input("Superficie m2", value=1, step=1, format="%d")
        precio_compra_m2 = st.number_input("Coste de compra del activo m2/€", value=1, step=1, format="%d")
        ...
        iajd = st.number_input("Impuesto Actos Jurídicos Documentados (%)", value=1.5, step=0.1, format="%.2f") / 100
        comercial_cost = st.number_input("Coste comercial (agencia inmobiliaria) (€)", value=0, step=1000, format="%d")
        precio_venta_m2 = st.number_input("Precio de venta del activo m2/€", value=1, step=1, format="%d")
        ...
        comision_apertura_interes = st.slider("Comisión de apertura (%)", min_value=0.0, max_value=6.0, value=3.0, step=0.1) / 100
        comision_apertura_com_int = st.slider("Comisión de apertura compartimento (%)", min_value=0.0, max_value=6.0, value=1.5, step=0.1) / 100
        comision_cierre_interes = st.slider("Comisión de cierre (%)", min_value=0.0, max_value=6.0, value=3.0, step=0.1) / 100
        comision_cierre_com_int = st.slider("Comisión de cierre compartimento (%)", min_value=0.0, max_value=6.0, value=1.5, step=0.1) / 100
        tasa_anual_deuda = st.slider("Tipo interés anual (%)", min_value=0.0, max_value=30.0, value=13.5, step=0.1) / 100
        success_fee = st.slider("Success fee(%)", min_value=0, max_value=30, value=20, step=1) / 100
        primera_disposicion_solicitada = st.number_input("Primera disposición solicitada (€)", value=0, step=1000, format="%d")
    ...
    max_iteraciones = 100
    iteraciones = 0
    acq_cost = m2 * precio_compra_m2
    ingresos_esperados = m2 * precio_venta_m2
    
    while tolerancia > 0.01 and iteraciones < max_iteraciones:
        deuda_total_pre = hard_costs + soft_costs_sin_fee + acq_cost - equity
        credere_fee = math.ceil((2.8 / 100) * deuda_total_pre / 1000) * 1000
    ...
    #Gastos
    incremento_gastos = 20/100
    gastos_notariales = (0.5/100 * deuda_total_pre)*(1+incremento_gastos)
    minuta_legal = (0.8/100 * deuda_total_pre)*(1+incremento_gastos)
    registro_propiedad = (0.4/100 * deuda_total_pre)*(1+incremento_gastos)
    comision_apertura = deuda_total * comision_apertura_interes
    gastos_iajd = deuda_total * iajd

    gastos_primera_dispocion = gastos_notariales + minuta_legal + registro_propiedad + gastos_iajd 
    ...
    disposición_inicial = gastos_primera_dispocion + comision_apertura if acq_cost - equity > 0 else primera_disposicion_solicitada + gastos_primera_dispocion + comision_apertura
    ...
    comision_cierre = deuda_total * comision_cierre_interes
    outflows[-1] += deuda_acumulada + intereses_acumulados + comision_cierre + comercial_cost
    ...
    # Ratios del inversor
    comision_cierre_com =  deuda_acumulada * comision_cierre_com_int
    free_cash_inver = intereses_acumulados + comision_cierre - comision_cierre_com
...
#GRAFICOS
    ...
    detalle_disposiciones_intereses = pd.DataFrame({
        ...
        "Comisión de Apertura (€)": [0] * (meses_totales - 1) + [comision_cierre]
    })
    ...
    comision_apertura, ROI_inv, ROI_inv_anualizado, logo_path
        ...
        # 1. Portada
        logo = ReportLabImage(logo_path, width=60, height=60)
        elementos.append(logo)
        elementos.append(Spacer(1, 20))
        elementos.append(Paragraph("Informe Financiero Credere Lending Capital", heading_style))
        elementos.append(Spacer(1, 20))
        ...
        elementos.append(Paragraph(f"<b>Propiedad:</b> Propiedad intelectual y comercial de Credere Capital INC", normal_style))
    ...
    if st.button("Generar Informe PDF"):
        pdf_generado = generar_pdf(
            ...
            (intereses_acumulados + comision_cierre + credere_fee) / deuda_total,
            EBIT / ingresos_esperados, deuda_total, outflows[-1], intereses_acumulados,
            comision_cierre, ROI_inv, ROI_inv_anualizado, logo_path
        )
