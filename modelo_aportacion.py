...
def aportacion():
    ...
    # =======================
    with st.sidebar:
        equity = st.number_input("Valor del inmueble aportado (€)", value=1000, step=1000, format="%d")
    ...
    while tolerancia > 0.01 and iteraciones < max_iteraciones:
        deuda_total_pre = hard_costs + soft_costs_sin_fee
        comision_levantamiento = (1.5 / 100) * deuda_total_pre #1.5% DE CAPITAL PRESTADO
        credere_fee = (
            (1.2 / 100) * hard_costs +
    ...
    total_costs_sin_deuda = hard_costs + soft_costs_con_fee + comercial_cost
    ...
    outflows[0] = disposicion_media_hard_cost + disposicion_media_soft_cost
    ...
    LTV_inicial = (inflows[0]) / tasacion
    LTC = (deuda_total) / (total_costs_sin_deuda + equity)
    ...
    with col_ratio:
        ...
        st.metric("Coste total sin intereses + activo", f"{total_costs_sin_deuda + equity:,.0f} €")
    ...
    with col_promotor:
        ...
        st.metric("Valor del activo aportado", f"{equity:,.0f} €")
        st.metric("Valor de tasación del activo",f"{tasacion:,.0f} €")
    ...
    with col_deuda:
        ...
        st.metric("Total adeudado con intereses", f"{outflows[-1]- comercial_cost:,.0f} €")
...
Costes Totales:                                   - € {total_costs_sin_deuda+equity:,.2f}
    Coste activo:                       - € {equity:,.2f}
...
EBITDA:                                              € {ingresos_esperados - total_costs_sin_deuda- equity:,.2f}
...
EBIT:                                                € {ingresos_esperados - total_costs_sin_deuda - equity - (intereses_acumulados + comision_apertura):,.2f}
"""
...
#GRAFICOS
    ...
    #coste_adquisicion = [equity if mes == 0 else 0 for mes in range(meses_totales)]
    hardcost_mes = [
    ...
    comision_apertura, ROI_inv, ROI_inv_anualizado, logo_path
        ...
        reporte_data = [
            ...
            [Paragraph("<b>Costes Totales</b>", styles["Normal"]), f"- € {total_costs_sin_deuda + equity:,.2f}"],
            ["      Coste de activo", f"      - € {equity:,.2f}"],
            ...
            [Paragraph("<b>EBITDA</b>", styles["Normal"]), f" € {ingresos_esperados - total_costs_sin_deuda - equity:,.2f}"],
            ...
            [Paragraph("<b>EBIT</b>", styles["Normal"]), f"  € {ingresos_esperados - total_costs_sin_deuda - equity - (intereses_acumulados + comision_apertura):,.2f}"]]
